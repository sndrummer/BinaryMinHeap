...
BinaryMinHeap::BinaryMinHeap() { initHeap(DEFAULT_SIZE); }

BinaryMinHeap::BinaryMinHeap(int maxSize) { initHeap(maxSize); }
...
BinaryMinHeap::~BinaryMinHeap() { delete[] heapArray; }

int BinaryMinHeap::getNodeCount() const { return nodeCount; }

int BinaryMinHeap::getLeftChildIndex(int nodeIndex) const { return 2 * nodeIndex; }

int BinaryMinHeap::getRightChildIndex(int nodeIndex) const { return 2 * nodeIndex + 1; }

int BinaryMinHeap::getParent(int nodeIndex) const { return int(floor((nodeIndex) / 2)); }

int BinaryMinHeap::getArraySize() const { return arraySize; }
...
void BinaryMinHeap::percolateDown()
{
    ...
    while (curIndex < nodeCount) {
        ...
        if (smallestChildVal < parentVal) {
            LOG_DBG("Swapping " + std::to_string(heapArray[curIndex]) + " and "
                + std::to_string(heapArray[smallestChildIndex]));
...
int BinaryMinHeap::getMin() const { return heapArray[HEAD]; }
...
bool BinaryMinHeap::hasParent(int index) const { return getParent(index) > 0; }

int BinaryMinHeap::getNextEmptyIndex() const { return nodeCount + 1; }
